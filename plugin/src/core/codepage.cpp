...
using HighTable = std::array<std::uint16_t, 128>;

/// Mappings for bytes 0x80..0xFF, generated from the system iconv tables.
/// 0xFFFD marks a byte the code page leaves undefined.
constexpr HighTable kWindows1250 = {
    0x20AC, 0xFFFD, 0x201A, 0xFFFD, 0x201E, 0x2026, 0x2020, 0x2021, 0xFFFD, 0x2030, 0x0160,
    0x2039, 0x015A, 0x0164, 0x017D, 0x0179, 0xFFFD, 0x2018, 0x2019, 0x201C, 0x201D, 0x2022,
    0x2013, 0x2014, 0xFFFD, 0x2122, 0x0161, 0x203A, 0x015B, 0x0165, 0x017E, 0x017A, 0x00A0,
...
constexpr HighTable kWindows1251 = {
...
constexpr HighTable kWindows1252 = {
...
const HighTable& tableFor(Codepage codepage)
{
    switch (codepage)
    {
...
void appendUtf8(std::string& out, std::uint16_t codePoint)
{
    if (codePoint < 0x80)
    {
...
{
    const std::size_t digits = text.find_last_not_of("0123456789");
    const std::string_view number =
        digits == std::string_view::npos ? text : text.substr(digits + 1);
    if (number == "1250")
    {
        out = Codepage::Windows1250;
        return true;
    }
    if (number == "1251")
    {
        out = Codepage::Windows1251;
        return true;
    }
    if (number == "1252")
    {
        out = Codepage::Windows1252;
        return true;
    }
    return false;
}
...
{
    const HighTable& table = tableFor(codepage);
    std::string out;
    out.reserve(text.size());
    for (const char raw : text)
    {
        const auto byte = static_cast<unsigned char>(raw);
        appendUtf8(out, byte < 0x80 ? byte : table[byte - 0x80u]);
    }
    return out;
}
