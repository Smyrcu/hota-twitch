...
import { creatureIcon, fortIcon, hallIcon, spellIcon, townPicture } from '../../data/sprites.js';
...
export function townHoverCard(town: Town): Card {
    ...
    const ops: DrawOp[] = [
        ...
        text(town.name, 'medium', 'yellow', HEADER.x, HEADER.nameY, 'left', HEADER.width),
        text(townTypeName(town.type), 'small', 'white', HEADER.x, HEADER.lineY, 'left', HEADER.width),
...
/**
 * Where the spell under research sits in the level's list. The list omits slots the faction
 * does not have, so the spell id is the reliable key; the slot index is the fallback.
 */
...
function guildSection(section: SectionBuilder, town: Town): void {
    ...
    for (let level = 1; level <= town.guild; level += 1) {
        ...
        spells.forEach((spell, index) => {
            ...
            const x = at.x + Math.floor((cell - iconWidth) / 2);
